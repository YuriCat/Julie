...
def make(x_placeholder):
    ...
    with tf.name_scope('fc1') as scope:
        W_fc1 = weight_variable([go.IMAGE_SIZE * N_FILTERS, N_FC_UNITS])
        b_fc1 = bias_variable([N_FC_UNITS])
        h_fc1 = tf.nn.relu(tf.matmul(h_last, W_fc1) + b_fc1)

    # 全結合層2
    with tf.name_scope('fc2') as scope:
        W_fc2 = weight_variable([N_FC_UNITS, 1])
        h_fc2 = tf.matmul(h_fc1, W_fc2)

    return tf.nn.tanh(h_fc2)

def calc_loss(value, labels):
    return tf.reduce_mean(tf.square(value - labels))
